...
def get_ger40():
    try:
        ...
        st.error(f"Fehler beim Laden der GER40-Daten: {e}")
        return pd.DataFrame()
...
# Signal-Analyse
    ...
    if file:
        df = pd.read_csv(file)
        st.write("🔍 Vorschau der Datei:", df.head())
        if "Signal" in df.columns and "Ergebnis" in df.columns:
            ...
            st.error("❌ Spalten 'Signal' und 'Ergebnis' fehlen in der Datei.")

# Hinweis
st.info("Diese Tools dienen nur zu Analysezwecken und stellen keine Anlageberatung dar.")
